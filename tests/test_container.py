...
def test_container_builds_mock_bundle():
    container = build_container(Settings(model_provider="mock", db_dsn="sqlite:///:memory:"))
...
def test_container_wires_same_repository():
    container = build_container(Settings(db_dsn="sqlite:///:memory:"))
...
def test_container_openai_compatible_builds_without_key():
    ...
    settings = Settings(model_provider="openai_compatible", openai_api_key=None)
