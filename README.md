...
- **演示模式（默认，开箱即用）**：不联网也能完整体验全流程——判断由内置规则引擎计算，想法评估与商业计划叙事使用模板；数据存在你自己的电脑里。
- **AI 模式（接入即增强）**：配置真实 AI 服务后，想法评估、商业计划叙事等由 AI 生成，判断的核心逻辑不变。这是 Vencertia 作为 skill 编排系统的完整形态——AI 出主意，规则管账本。
...
> 刚装好时是**演示模式**：判断由内置规则引擎计算，想法评估与商业计划叙事使用模板，全程离线。想启用 AI 生成叙事，见 [常见问题](#需要接入-ai-接口吗)。
...
- **演示模式（默认）**：不需要。判断由内置规则引擎完成，想法评估与商业计划用模板生成，全流程离线可用。
- **AI 模式**：接入后更完整。配置一个兼容 OpenAI 的接口（地址 + 密钥）即可，Vencertia 的 skill 编排层会把"想法评估、商业计划叙事"交给 AI 生成，而判断与账本始终由规则引擎负责。
